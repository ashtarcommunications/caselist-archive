var XWiki = (function(XWiki) {
  ...
  var FileUploadItem = Class.create({
    ...
    generateStatusUI : function() {
      ...
      if (this.options.enableFileInfo) {
        ...
        statusUI.FILE_CANCEL = UploadUtils.createButton("Cancel upload", this.cancelUpload.bindAsEventListener(this));
        // TODO MIME type icon?
    ...
    cancelUpload : function(event) {
      ...
      this.statusUI.FILE_CANCEL.addClassName('upload-canceled-label').removeClassName('buttonwrapper').update("Canceled");
  ...
  XWiki.FileUploader = Class.create({
    ...
    messages : {
      UNKNOWN_ERROR         : new Template("An error occurred while uploading #{name}"),
      INVALID_FILE_TYPE     : new Template("The file #{name} has an unsuported format"),
      UPLOAD_LIMIT_EXCEEDED : new Template("The file #{name} is too large. Please choose files under #{size}"),
      UPLOAD_ABORTED        : new Template("The upload of #{name} has been canceled"),
      UPLOAD_FINISHING      : new Template("Waiting for server confirmation for #{name}..."),
      UPLOAD_FINISHED       : new Template("Attachment uploaded: #{name} (#{size})")
    },
